...
// ── Estado del módulo ──
// ═══════════════════════════════════════════════════════════════
// NORMALIZACIÓN DE CLAVE AGEB
// ─────────────────────────────────────────────────────────────
// Las fuentes de datos usan distintos formatos de clave AGEB:
//   CONEVAL 2015 (coneval_irapuato.json)  : 14 dígitos
//   CONEVAL 2020 (coneval_ageb_2020.json) : 13 dígitos (sin dígito loc)
//   Censo 2020   (censo_ageb_11017.csv)   : 14 dígitos
//   GeoJSON INEGI (agebs_irapuato.geojson): 14 dígitos (CVEGEO)
//
// REGLA CANÓNICA: siempre operar en 14 dígitos internamente.
// Al buscar en coneval2020 (13 dígitos), convertir justo antes del lookup.
// ═══════════════════════════════════════════════════════════════

/**
 * Normaliza una clave AGEB al formato de 14 digitos (formato canonico).
 * @param {string|number} clave  - Clave en cualquier formato
 * @returns {string} Clave de exactamente 14 digitos, o '' si invalida
 */
function normalizarClaveAGEB(clave) {
  if (!clave) return '';
  var s = String(clave).replace(/\./g, '').trim();
  if (s.length === 13) {
    // CONEVAL 2020: le falta el digito de localidad (pos 8)
    // Formato: 2(ent) + 3(mun) + 4(ageb) + 4(id) = 13
    // Formato 14: 2(ent) + 3(mun) + 4(loc='0001') + 4(ageb) — insertar '0' en pos 8
    // Sin embargo la estructura exacta varia; la forma mas segura es
    // anteponer '0' al final del segmento municipal: pos 5 -> insertar en pos 8
    s = s.slice(0, 8) + '0' + s.slice(8);
  }
  if (s.length === 15) {
    // GeoJSON con digito de manzana — quitar el ultimo
    s = s.slice(0, 14);
  }
  if (s.length !== 14) return '';
  return s;
}
window.normalizarClaveAGEB = normalizarClaveAGEB;

/**
 * Convierte clave de 14 a 13 digitos para lookup en coneval_ageb_2020.json
 */
function claveAGEB13(clave14) {
  var s = normalizarClaveAGEB(clave14);
  if (!s) return '';
  // Quitar el '0' insertado en pos 8
  return s.slice(0, 8) + s.slice(9);
}
window.claveAGEB13 = claveAGEB13;


var GEO = {
  coneval:     null,   // datos CONEVAL 2015 (rango pobreza por AGEB)
  coneval2020: null,   // datos CONEVAL 2020 (grado rezago + 17 indicadores)
...
// Paleta CONEVAL 2020 — 5 grados de rezago social
var AGEB_COLORES_2020 = {
  'Muy bajo': { fill: '#1a6e3c', stroke: '#14532d', label: 'Muy bajo' },
  'Bajo':     { fill: '#d97706', stroke: '#92400e', label: 'Bajo'     },
  'Medio':    { fill: '#dc2626', stroke: '#991b1b', label: 'Medio'    },
  'Alto':     { fill: '#9f1239', stroke: '#7f1d1d', label: 'Alto'     },
  'Muy alto': { fill: '#581c87', stroke: '#3b0764', label: 'Muy alto' }
};
...
function geoCargar(callback) {
  ...
  fetch('coneval_irapuato.json')
    ...
    .then(function(data) {
      GEO.coneval = data;
      // Cargar CONEVAL 2020 en paralelo (no bloquea)
      fetch('coneval_ageb_2020.json')
        .then(function(r2) { return r2.json(); })
        .then(function(data2) {
          // Indexar por clave_ageb de 13 dígitos
          var idx = {};
          data2.forEach(function(row) {
            if (row.clave_ageb) idx[row.clave_ageb] = row;
          });
          GEO.coneval2020 = idx;
          console.log('[GEO] CONEVAL 2020 cargado: ' + Object.keys(idx).length + ' AGEBs');
        })
        .catch(function(e) { console.warn('[GEO] coneval_ageb_2020.json no disponible:', e.message); });
...
function geoRenderCapaAgebs(mapaLeaflet) {
  ...
  var coneval2020 = GEO.coneval2020 || {};
  var usar2020    = Object.keys(coneval2020).length > 0;
  ...
  GEO.agebLayer = L.geoJSON(GEO.geojson, {
    ...
    onEachFeature: function(feature, layer) {
      var clave = _extraerClaveAgeb(feature);
      var datos2020p = (GEO.coneval2020 || {})[clave];
      ...
      var popupHtml =
        ...
        '</div>' +
        (datos2020p && datos2020p.grado_rezago
          ? '<div style="font-size:9px;color:#00ff88;font-weight:bold;margin-bottom:3px;">Rezago 2020: ' + datos2020p.grado_rezago + '</div>' +
            '<div style="font-size:8px;color:#7a9ab8;margin-bottom:1px;">Sin salud: ' + (datos2020p.sin_salud ? parseFloat(datos2020p.sin_salud).toFixed(1) + '%' : 'N/D') + '</div>' +
            '<div style="font-size:8px;color:#7a9ab8;margin-bottom:1px;">Sin internet: ' + (datos2020p.sin_internet ? parseFloat(datos2020p.sin_internet).toFixed(1) + '%' : 'N/D') + '</div>' +
            '<div style="font-size:8px;color:#7a9ab8;margin-bottom:2px;">Hacinamiento: ' + (datos2020p.hacinamiento ? parseFloat(datos2020p.hacinamiento).toFixed(1) + '%' : 'N/D') + '</div>'
          : '<div style="font-size:8px;color:#7a9ab8;margin-bottom:2px;">Rango 2015: ' + (datos ? datos.rango : 'Sin datos') + '</div>') +
...
function geoToggleAgebs(mapaLeaflet, btnEl) {
  ...
  if (typeof toast === 'function') {
    toast(GEO.agebActivo ? '📊 Rezago Social 2020 · 230 AGEBs' : 'Capa CONEVAL desactivada', 'ok');
  }
}
...
function _mostrarLeyendaAgebs(mapaLeaflet) {
  ...
  leyenda.onAdd = function() {
    ...
    div.innerHTML = '<div style="font-size:7px;color:#3a6a9a;letter-spacing:1px;margin-bottom:6px;">POBREZA POR AGEB · CONEVAL 2015</div>' +
      Object.keys(AGEB_COLORES_2020).map(function(k) {
        var c = AGEB_COLORES_2020[k];
...
// ─── Registro de proveedores de contexto IA ──────────────────────────────────
// Se ejecuta al cargar geo.js. IA_CONTEXT_PROVIDERS puede no existir aun si
// ia.js no cargo primero — usar lazy init con getter al momento de llamada.
(function() {
  window.IA_CONTEXT_PROVIDERS = window.IA_CONTEXT_PROVIDERS || [];

  // Proveedor 1: contexto geo + IRZ (geoTextoParaIA + analisisContextoIA)
  window.IA_CONTEXT_PROVIDERS.push(function(lat, lng) {
    var parts = [];
    if (typeof geoTextoParaIA === 'function') {
      var g = geoTextoParaIA(lat, lng);
      if (g) parts.push(g);
    }
    if (typeof analisisContextoIA === 'function') {
      var irz = analisisContextoIA(lat, lng);
      if (irz) parts.push(irz);
    }
