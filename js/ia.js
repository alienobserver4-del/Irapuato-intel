...
// ─── Sistema de proveedores de contexto para buildPrompt() ───────────────────
// Cada modulo que quiera inyectar contexto en el prompt de IA se registra aqui.
// Firma: function(lat, lng) -> string | null
// Ejemplo desde geo.js:
//   window.IA_CONTEXT_PROVIDERS.push(function(lat, lng) {
//     return geoTextoParaIA(lat, lng);
//   });
// buildPrompt() itera el array — nunca necesita conocer a los modulos por nombre.
window.IA_CONTEXT_PROVIDERS = [];
// Limite de tokens por proveedor (aprox chars) — evita prompts que crecen sin control
var IA_CONTEXT_MAX_CHARS = 400;
...
function buildPrompt(texto) {
  ...
  p += '14. subtipo_seguridad: SOLO si tipo="seguridad", clasifica en uno de: homicidio_doloso, homicidio_culposo, feminicidio, lesiones_dolosas, lesiones_culposas, robo_con_violencia, robo_sin_violencia, robo_vehiculo, robo_casa, robo_negocio, robo_transeunte, secuestro, extorsion, narcomenudeo, portacion_armas, privacion_libertad, desaparicion_forzada, violacion, violencia_familiar, abuso_sexual, amenazas, fraude, otro_seguridad. Si no es seguridad, pon "".\n\n';

  // Contexto territorial — iterar proveedores registrados (ver IA_CONTEXT_PROVIDERS)
  if (window.IA_CONTEXT_PROVIDERS && window.IA_CONTEXT_PROVIDERS.length > 0 && window._iaGeoLat && window._iaGeoLng) {
    var ctxParts = [];
    for (var _pi = 0; _pi < window.IA_CONTEXT_PROVIDERS.length; _pi++) {
      try {
        var _ctx = window.IA_CONTEXT_PROVIDERS[_pi](window._iaGeoLat, window._iaGeoLng);
        if (_ctx && typeof _ctx === 'string' && _ctx.trim()) {
          // Respetar limite de chars por proveedor
          ctxParts.push(_ctx.trim().slice(0, IA_CONTEXT_MAX_CHARS));
        }
      } catch(e) { /* proveedor fallido — ignorar silenciosamente */ }
    }
    if (ctxParts.length > 0) {
      p += 'CONTEXTO TERRITORIAL:\n' + ctxParts.join('\n') + '\n\n';
    }
  }
  ...
  // Sprint 6: instrucción de subtipo de seguridad
  if (typeof subtipoSegPromptExtra === 'function') {
    p = p.replace('=== ANALIZA SOLO', subtipoSegPromptExtra() + '\n=== ANALIZA SOLO');
  }
