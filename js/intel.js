...
// ── Sprint 6: Icono con tamaño dinámico según zoom del mapa Intel ──
function _intelIconoTamano() {
  if (!intelObj) return { w:16, h:20 };
  var z = intelObj.getZoom();
  if (z <= 11) return { w:10, h:13 };
  if (z <= 13) return { w:14, h:18 };
  if (z <= 15) return { w:18, h:23 };
  return { w:22, h:28 };
}

// Override de crearIconoMapa para Intel — usa tamaño dinámico por zoom
function crearIconoMapa(tipo) {
  var color = (typeof COLORES_TIPO !== 'undefined' && COLORES_TIPO[tipo]) ? COLORES_TIPO[tipo] : '#3a5a7a';
  var t = _intelIconoTamano();
  var svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + t.w + '" height="' + t.h +
    '" viewBox="0 0 22 28">' +
  ...
  return L.divIcon({
    ...
    className: '',
    iconSize:    [t.w, t.h],
    iconAnchor:  [Math.round(t.w/2), t.h],
    popupAnchor: [0, -(t.h)]
  });
}

// ── Icono de tamaño dinámico según nivel de zoom ──
// zoom 10-11 → 10px, zoom 12 → 14px, zoom 13 → 16px, zoom 14 → 18px, zoom 15+ → 22px
function _intelIconSize(zoom) {
...
function crearIconoIntel(tipo, zoom) {
  var color = COLORES_TIPO[tipo] || '#3a5a7a';
  var sz = _intelIconSize(zoom || 12);
  var szH = Math.round(sz * 1.27); // proporción altura del pin
  var svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + sz + '" height="' + szH + '" viewBox="0 0 22 28">' +
  ...
  return L.divIcon({
    ...
    className: '',
    iconSize: [sz, szH],
    iconAnchor: [Math.round(sz/2), szH],
    popupAnchor: [0, -szH]
  });
}
...
function iniciarIntel() {
  try {
    ...
    // Sprint 6: re-renderizar markers al cambiar zoom para ajustar tamaño de iconos
    intelObj.on('zoomend', function() {
...
function renderIntel() {
  ...
  for (var gi = 0; gi < grupos.length; gi++) {
    ...
    var _zoom = intelObj ? intelObj.getZoom() : 12;
    var marker = L.marker([lat, lng], {icon: crearIconoIntel(tipo, _zoom)});
    var _nId = n.id || '';
    ...
    var popup =
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">' +
        ...
        '<div style="display:flex;gap:4px;">' +
          ...
          '<button onclick="irACorpus(\'' + _nId + '\')" style="font-family:\'Orbitron\',monospace;font-size:7px;padding:3px 7px;background:rgba(176,96,255,.15);color:#b060ff;border:1px solid #b060ff66;border-radius:2px;cursor:pointer;letter-spacing:1px;">&#128230; CORPUS</button>' +
        '</div>' +
      '</div>' +
      (lat && lat !== 20.6795 && typeof analisisBadgeHTML === 'function' ? analisisBadgeHTML(lat, lng) + ' ' : '') +
      (lat && lat !== 20.6795 && typeof geoLookup === 'function' && typeof prediccionBadgeHTML === 'function' ? (function(){ var _pg = geoLookup(lat, lng); return _pg && _pg.clave_ageb ? prediccionBadgeHTML(_pg.clave_ageb) : ''; })() : '') +
...
function posicionarEnIntel(id) {
  ...
  // Si las coords son el centro genérico o inválidas, intentar con calles.js primero, luego Nominatim
  var esCentroGenerico = (Math.abs(lat - 20.6795) < 0.001 && Math.abs(lng - (-101.3540)) < 0.001);
  if ((!lat || !lng || esCentroGenerico) && (calle1 || colonia)) {

    // Sprint 7: consultar base interna de calles primero
    if (typeof callesLookupCompleto === 'function') {
      var calleLocal = callesLookupCompleto(calle1, colonia);
      if (calleLocal && calleLocal.lat) {
        if (status) { status.textContent = '✅ Base interna: ' + calleLocal.nombre_oficial; status.style.color = '#00ff88'; }
        if (db) db.collection('noticias-fase1').doc(id).update({ lat: calleLocal.lat, lng: calleLocal.lng });
        for (var i2 = 0; i2 < noticias.length; i2++) {
          if (noticias[i2].id === id) { noticias[i2].lat = calleLocal.lat; noticias[i2].lng = calleLocal.lng; break; }
        }
        _colocarEnIntel(id, titulo, tipo, calle1, colonia || calleLocal.colonia, resumen, fecha, calleLocal.lat, calleLocal.lng);
        return;
      }
    }

    // Si no está en base interna, intentar Nominatim
    ...
    fetch(url)
      ...
      .then(function(data) {
        if (data && data[0]) {
          ...
          if (status) { status.textContent = '✅ Nominatim: ' + (data[0].display_name||'').split(',').slice(0,2).join(','); status.style.color = '#00ff88'; }
          _colocarEnIntel(id, titulo, tipo, calle1, colonia, resumen, fecha, lat, lng);
        } else {
          // Sprint 7: NO colocar en centro si no encuentra
          if (status) { status.textContent = '⚠ No encontrado — ingresa las coordenadas manualmente'; status.style.color = '#ff8800'; }
          toast('No se encontró la ubicación. Ingresa coordenadas manualmente.', 'warn');
        }
      })
      .catch(function() {
        if (status) { status.textContent = '✗ Error de red'; status.style.color = '#ff2255'; }
      });
    toast('Geocodificando ubicación...', 'ok');
    return;
  }

  if (!lat || !lng || esCentroGenerico) {
    toast('Sin coordenadas — ingresa lat/lng manualmente en el panel de edición', 'warn');
    return;
  }
  _colocarEnIntel(id, titulo, tipo, calle1, colonia, resumen, fecha, lat, lng);
}
...
function _colocarEnIntel(id, titulo, tipo, calle1, colonia, resumen, fecha, lat, lng) {
  ...
  setTimeout(function() {
    ...
    var color = COLORES_TIPO[tipo] || '#3a5a7a';
    var _zoom2 = intelObj ? intelObj.getZoom() : 15;
    var marker = L.marker([lat, lng], { icon: crearIconoIntel(tipo, _zoom2) });
    ...
    var popup =
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">' +
        ...
        '<div style="display:flex;gap:4px;">' +
          ...
          '<button onclick="irACorpus(\'' + id + '\')" style="font-family:\'Orbitron\',monospace;font-size:7px;padding:3px 7px;background:rgba(176,96,255,.15);color:#b060ff;border:1px solid #b060ff66;border-radius:2px;cursor:pointer;letter-spacing:1px;">&#128230; CORPUS</button>' +
        '</div>' +
      '</div>' +
...
// ── Navegar desde popup Intel → BD Corpus, resaltando la tarjeta ──
function irACorpus(id) {
  // Cerrar popup del mapa
  if (intelObj) intelObj.closePopup();
  // Cambiar a pestaña BD
  if (typeof verTab === 'function') verTab('bd');
  // Activar subpestaña corpus
  setTimeout(function() {
    var btnCorpus = document.querySelector('.subtab-btn[onclick*="corpus"]');
    if (btnCorpus) btnCorpus.click();
    // Esperar render y hacer scroll a la tarjeta
    setTimeout(function() {
      var card = document.getElementById('bd-' + id);
      if (card) {
        card.scrollIntoView({ behavior: 'smooth', block: 'center' });
        // Flash de resaltado para ubicar visualmente la tarjeta
        card.style.transition = 'box-shadow .2s';
        card.style.boxShadow = '0 0 0 2px #b060ff, 0 0 20px #b060ff55';
        setTimeout(function() { card.style.boxShadow = ''; }, 1800);
      }
    }, 350);
  }, 200);
}
window.irACorpus = irACorpus;
