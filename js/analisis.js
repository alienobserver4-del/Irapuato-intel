...
function _componentePobreza(clave_ageb) {
  ...
  // Normalizar a 14 digitos (formato canonico de coneval_irapuato.json)
  var clave = (typeof normalizarClaveAGEB === 'function') ? normalizarClaveAGEB(clave_ageb) : clave_ageb;
...
function calcularIndiceRiesgo(clave_ageb) {
  ...
  // Normalizar a 14 digitos al entrar — garantiza hits consistentes en cache
  if (typeof normalizarClaveAGEB === 'function') clave_ageb = normalizarClaveAGEB(clave_ageb) || clave_ageb;
  ...
  // Componente Censo 2020 (IVC)
  var c = 0;
  if (typeof CENSO !== 'undefined' && CENSO.loaded) {
    var dc = censoLookup(clave_ageb);
    if (dc && dc._ivc !== null) c = dc._ivc;
  }
  ...
  var resultado = {
    ...
    label:     nivel.label,
    emoji:     nivel.emoji,
    comp: { pobreza: Math.round(p), censo: Math.round(c), delitos: Math.round(d), noticias: Math.round(n) }
  };
...
function _claveDeFeature(feature) {
  ...
  // Usar normalizacion centralizada de geo.js
  if (typeof normalizarClaveAGEB === 'function') return normalizarClaveAGEB(raw);
